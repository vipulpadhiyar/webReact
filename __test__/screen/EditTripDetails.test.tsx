...
import {AppScreens, EditProfileDetailsScreen} from '~/constants';
import {EditTripDetails} from '~/screens';

import {renderWithProviders} from '../utils/TestUtils';

// Mock useForgotPasswordController
jest.mock('../../src/screens/EditTripDetails/controller.ts', () => ({
...
describe('Home Screen', () => {
  it('renders correctly', () => {
    const {getByTestId} = renderWithProviders([
      {name: AppScreens.EditProfileScreen, screen: EditTripDetails},
    ]);

    // Assert presence of UI elements based on testIDs or other unique identifiers
    expect(
      getByTestId(EditProfileDetailsScreen.EDIT_PROFILE_SCREEN),
    ).toBeDefined();
    expect(
      getByTestId(EditProfileDetailsScreen.EDIT_PROFILE_NAME),
    ).toBeDefined();
    expect(
      getByTestId(EditProfileDetailsScreen.EDIT_PROFILE_PICK_UP_DATE),
    ).toBeDefined();
    expect(
      getByTestId(EditProfileDetailsScreen.EDIT_PROFILE_PICK_UP_ADDRESS),
    ).toBeDefined();
    expect(
      getByTestId(EditProfileDetailsScreen.EDIT_PROFILE_DROP_UP_ADDRESS),
    ).toBeDefined();
    expect(
      getByTestId(EditProfileDetailsScreen.EDIT_PROFILE_NOTES),
    ).toBeDefined();
    expect(
      getByTestId(EditProfileDetailsScreen.EDIT_PROFILE_RETURN_DROP_UP_ADDRESS),
    ).toBeDefined();
    expect(
      getByTestId(EditProfileDetailsScreen.EDIT_PROFILE_RETURN_PICK_UP_ADDRESS),
    ).toBeDefined();
    expect(
      getByTestId(EditProfileDetailsScreen.EDIT_PROFILE_RETURN_PICK_UP_DATE),
    ).toBeDefined();
    ...
    expect(
      getByTestId(EditProfileDetailsScreen.EDIT_PROFILE_SINGLE_TRIP_BUTTON),
    ).toBeDefined();
    expect(
      getByTestId(EditProfileDetailsScreen.EDIT_PROFILE_ROUND_TRIP_BUTTON),
    ).toBeDefined();
...
describe('Home Screen Test Suit 2', () => {
  it('renders AppScrollView when isDisplayPickUpList is true and autoCompleteAddressList is populated', () => {
    const {getByTestId} = renderWithProviders([
      {name: AppScreens.EditTripDetails, screen: EditTripDetails},
    ]);

    // Assert that AppScrollView is rendered
    const appScrollPickup = getByTestId(
      EditProfileDetailsScreen.EDIT_PROFILE_SCROLL_PICK_UP,
    ); // Adjust with your test ID
    expect(appScrollPickup).toBeTruthy();

    const appScrollDrop = getByTestId(
      EditProfileDetailsScreen.EDIT_PROFILE_SCROLL_DROP,
    ); // Adjust with your test ID
    expect(appScrollDrop).toBeTruthy();

    const appScrollRoundPickup = getByTestId(
      EditProfileDetailsScreen.EDIT_PROFILE_ROUND_PICK_UP,
    ); // Adjust with your test ID
    expect(appScrollRoundPickup).toBeTruthy();

    const appScrollRoundDrop = getByTestId(
      EditProfileDetailsScreen.EDIT_PROFILE_ROUND_DROP,
    ); // Adjust with your test ID
